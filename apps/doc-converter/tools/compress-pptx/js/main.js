...
const API_URL = 'https://toolbox-backend-76dc.onrender.com/convert/compress-pptx';
...
function addFiles(files) {
  ...
  const pptCount = files.filter(f => {
    const name = f.name.toLowerCase();
    return name.endsWith('.pptx') || name.endsWith('.ppt');
  }).length;
  const otherCount = files.length - pptCount;
  let label = `${files.length} file${files.length !== 1 ? 's' : ''} selected`;
  if (otherCount > 0) label += ` · ${otherCount} non-PowerPoint will be skipped or returned as-is`;
