/**
 * Chronos Toolbox — Video to Audio
 * Backend-integrated tool that extracts MP3 from upload videos using FFmpeg.
 */

// ── API URL configuration ────────────────────────────────
const isLocal = window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1';
const BASE_URL = isLocal ? 'http://localhost:8000' : 'https://toolbox-backend-76dc.onrender.com';
const API_URL = `${BASE_URL}/media/video-to-audio`;
...
async function ensureBackendAwake() {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      progressText.textContent = 'Uploading and extracting audio...';
      return true;
    } catch (e) {
...
btnExtract.addEventListener('click', async () => {
  ...
  try {
    ...
    btnDownload.download = selectedFile.name.replace(/\.[^/.]+$/, "") + ".mp3";
    ...
    showToast('Audio extraction successful!');
  } catch (err) {
    console.error(err);
    showToast(err.message || 'Failed to extract audio.', true);
    clearInterval(progressInterval);
    progressWrap.classList.remove('visible');
    actionBar.style.display = 'flex';
  } finally {
